...
class ExporterService:
    ...
    def export_works(self, db: Session, work_ids: List[str] = None,
        ...
        # 表头
        headers = [
            "作品ID", "标题", "类型", "作者", "点赞数", "评论数",
            "收藏数", "分享数", "发布时间", "采集时间"
        ]
        ws.append(headers)
        self._style_header(ws, 1, len(headers))
        self._set_column_widths(ws, [20, 50, 10, 20, 12, 12, 12, 12, 20, 20])
    ...
    def export_users(self, db: Session, uids: List[str] = None,
        ...
        ws.append(headers)
        self._style_header(ws, 1, len(headers))
        self._set_column_widths(ws, [25, 25, 15, 15, 12, 15, 15, 20])
